"""
build_qg_dataset_v13.py
AQPG Phase 13 Master Dataset Ingestion, Normalization, Verification & Rebuild Engine.

Executes 20 Sequential Pipeline Stages:
  1. Source Discovery across raw archives and datasets/raw/v13/
  2. Source Ingestion
  3. Provenance & License Tracking
  4. Schema Normalization
  5. Subject Normalization (Strict Categorization)
  6. Class Grounding (Deterministic Evidence-based)
  ...
  19. Master V13 Dataset Assembly (datasets/v13/qg_dataset_v13.jsonl)
  20. Deterministic Leakage-Safe 80/20 Train/Val Split (seed 42)

Outputs:
  - datasets/v13/qg_dataset_v13.jsonl
  - datasets/v13/qg_train_dataset_v13.jsonl
  - datasets/v13/qg_validation_dataset_v13.jsonl
  - datasets/v13/qg_dataset_v13_report.txt
"""
...
RAW_V13_DIR = os.path.join(DATASETS_DIR, "raw", "v13")
...
REPORT_V13_PATH = os.path.join(V13_DIR, "qg_dataset_v13_report.txt")
...
def construct_v13_prompt(rec: dict) -> str:
    """Constructs structured encoder prompt for V13 Master Schema."""
...
def run_v13_pipeline():
    print("=" * 80)
    print("AQPG PHASE 13 MASTER DATASET PIPELINE (20 STAGES)")
    print("=" * 80)

    all_raw_records = []

    # 1. Ingest newly acquired Phase 13 JSON datasets from datasets/raw/v13
    v13_files = [
        "sciq_dataset_raw.json",
        "ai2_arc_raw.json",
        "mmlu_stem_raw.json",
        "openbookqa_raw.json",
        "ncert_stem_class9_12_raw.json"
    ]
    
    for vf in v13_files:
        p = os.path.join(RAW_V13_DIR, vf)
        if os.path.exists(p):
            with open(p, "r", encoding="utf-8") as f:
                ...
                for item in items:
                    all_raw_records.append({
                        ...
                        "provenance": item.get("provenance", "Phase 13 Ingested Dataset"),
    ...
    for idx, r in enumerate(all_raw_records, 1):
        ...
        if "math" in sb_raw.lower():
            subject = "Mathematics"
        elif "physic" in sb_raw.lower():
            subject = "Physics"
        ...
        master_rec["input_text"] = construct_v13_prompt(master_rec)
        v13_records.append(master_rec)

    total_v13 = len(v13_records)
    print(f"\nSuccessfully Constructed {total_v13} Clean V13 Records.")
    print("Rejection Breakdown:")
    for reason, count in rejection_stats.items():
        print(f"  - {reason}: {count}")

    # Save Master V13 Dataset
    with open(MASTER_V13_PATH, "w", encoding="utf-8") as f:
        for r in v13_records:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")
    print(f"\n[SUCCESS] Saved Master V13 to: {MASTER_V13_PATH} ({total_v13} records)")

    # Deterministic Leakage-Safe 80/20 Train / Val Split (seed 42)
    random.seed(42)
    shuffled = v13_records.copy()
    random.shuffle(shuffled)
    
    split_idx = int(0.80 * total_v13)
    train_v13 = shuffled[:split_idx]
    val_v13 = shuffled[split_idx:]

    with open(TRAIN_V13_PATH, "w", encoding="utf-8") as f:
        for r in train_v13:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")

    with open(VAL_V13_PATH, "w", encoding="utf-8") as f:
        for r in val_v13:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")

    print(f"[SUCCESS] Saved Train V13 to:  {TRAIN_V13_PATH} ({len(train_v13)} records)")
    print(f"[SUCCESS] Saved Val V13 to:    {VAL_V13_PATH} ({len(val_v13)} records)")

    # Build Report
    subject_counts = Counter(r["subject"] for r in v13_records)
    class_counts = Counter(r["class"] for r in v13_records)
    board_counts = Counter(r["board"] for r in v13_records)
    status_counts = Counter(r["verification_status"] for r in v13_records)

    report_lines = [
        "================================================================================",
        "AQPG PHASE 13 MASTER DATASET PIPELINE BUILD REPORT",
        "================================================================================",
        f"Total Ingested Raw Candidate Records: {len(all_raw_records)}",
        f"Total Clean V13 Records Constructed:  {total_v13}",
        f"Total Rejected Records:               {len(all_raw_records) - total_v13}\n",
        "TRAIN / VAL LEAKAGE-SAFE SPLIT (Seed 42):",
        f"  - Training Set V13 (80%):    {len(train_v13)} records",
        f"  - Validation Set V13 (20%):  {len(val_v13)} records\n",
        "SUBJECT DISTRIBUTION IN V13:"
    ]
    for sb, count in subject_counts.items():
        report_lines.append(f"  - {sb}: {count} ({(count/total_v13*100):.2f}%)")

    report_lines.extend(["\nCLASS DISTRIBUTION IN V13:"])
    for cl, count in class_counts.items():
        report_lines.append(f"  - {cl}: {count} ({(count/total_v13*100):.2f}%)")

    report_lines.extend(["\nBOARD DISTRIBUTION IN V13:"])
    for bd, count in board_counts.items():
        report_lines.append(f"  - {bd}: {count} ({(count/total_v13*100):.2f}%)")

    report_lines.extend(["\nVERIFICATION STATUS BREAKDOWN IN V13:"])
    for st, count in status_counts.items():
        report_lines.append(f"  - {st}: {count} ({(count/total_v13*100):.2f}%)")

    with open(REPORT_V13_PATH, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f"[SUCCESS] Saved Pipeline Build Report to: {REPORT_V13_PATH}")
