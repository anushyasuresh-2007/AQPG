"""
audit_raw_sources_v15.py
AQPG Phase 15 Raw Dataset Forensic Inventory Engine.

Scans all raw files in `datasets/raw/v15/` and extracted archives.
Generates:
  - datasets/phase15_source_inventory.json
  - datasets/phase15_source_inventory_report.txt
"""
...
RAW_V15_DIR = os.path.join(RAW_DIR, "v15")
EXTRACTED_DIR = os.path.join(DATASETS_DIR, "extracted")

JSON_OUT = os.path.join(DATASETS_DIR, "phase15_source_inventory.json")
REPORT_OUT = os.path.join(DATASETS_DIR, "phase15_source_inventory_report.txt")
...
def analyze_json_raw_file(filepath: str, source_name: str, source_url: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "download_method": "Automated OpenStax & Hugging Face Repository Ingestion"
    }

def run_v15_raw_inventory():
    print("=" * 80)
    print("AQPG PHASE 15 RAW DATASET FORENSIC INVENTORY")
    print("=" * 80)
    
    inventory = []
    
    # 1. Scan Phase 15 JSON files in datasets/raw/v15
    v15_sources = [
        ("openstax_physics_raw.json", "OpenStax University & College Physics", "https://openstax.org/subjects/science"),
        ("mmlu_stem_expanded_raw.json", "MMLU STEM & K-12 Curriculum Subsets", "https://huggingface.co/datasets/cais/mmlu"),
        ("ai2_arc_grounded_raw.json", "AI2 ARC Benchmark with Grade 9 & 10 Grounding", "https://huggingface.co/datasets/allenai/ai2_arc"),
        ("science_benchmarks_raw.json", "SciQ & OpenBookQA Benchmarks", "https://huggingface.co/datasets/allenai/sciq"),
        ("ncert_exemplar_class9_12_raw.json", "NCERT Exemplar & Benchmark STEM (Class 9-12)", "https://ncert.nic.in/exemplar-problems.php")
    ]
    
    for fn, name, url in v15_sources:
        p = os.path.join(RAW_V15_DIR, fn)
        if os.path.exists(p):
    ...
    print(f"\n[SUCCESS] Saved Phase 15 JSON inventory to: {JSON_OUT}")

    # Save Text Inventory Report
    report_lines = [
        "================================================================================",
        "AQPG PHASE 15 RAW SOURCE INVENTORY REPORT",
        "================================================================================",
    ...
    with open(REPORT_OUT, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f"[SUCCESS] Saved Phase 15 Text inventory report to: {REPORT_OUT}")

if __name__ == "__main__":
    run_v15_raw_inventory()
