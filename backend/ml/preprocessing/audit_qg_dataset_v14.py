"""
audit_qg_dataset_v14.py
AQPG Phase 14 Forensic Audit, Quality Gates, Cross-Tabulation & Tokenization Engine.

Performs Comprehensive Audit across:
  - datasets/v14/qg_dataset_v14.jsonl (Master)
  - datasets/v14/qg_train_dataset_v14.jsonl (Train)
  - datasets/v14/qg_validation_dataset_v14.jsonl (Val)
...
Executes:
  ...
  - Hard/Soft Quality Gates & Empirical Readiness Evaluation

Outputs:
  - datasets/v14/qg_dataset_v14_audit.json
  - datasets/v14/qg_dataset_v14_audit_report.txt
"""
...
AUDIT_JSON_PATH = os.path.join(V14_DIR, "qg_dataset_v14_audit.json")
AUDIT_REPORT_PATH = os.path.join(V14_DIR, "qg_dataset_v14_audit_report.txt")
...
def run_v14_audit():
    print("=" * 80)
    print("AQPG PHASE 14 DATASET FORENSIC AUDIT & QUALITY GATES")
    print("=" * 80)
    
    train_records, train_errs = load_and_validate_jsonl(TRAIN_V14_PATH)
    val_records, val_errs = load_and_validate_jsonl(VAL_V14_PATH)
    ...
    print(f"Total V14 Records:   {total_records}")
    ...
    if not c9_pass:
        gate_warnings.append(f"Class 9 Threshold Gap: {c9_cnt} < 1,000 records.")
    if not c10_pass:
        gate_warnings.append(f"Class 10 Threshold Gap: {c10_cnt} < 1,000 records.")
    if not c11_pass:
        gate_warnings.append(f"Class 11 Threshold Gap: {c11_cnt} < 1,000 records.")
    if not c12_pass:
        gate_warnings.append(f"Class 12 Threshold Gap: {c12_cnt} < 1,000 records.")
    ...
    elif math_pass and phy_pass and chem_pass and c9_pass and c10_pass and c11_pass and c12_pass and norm_target_leakage == 0 and dup_pairs_count == 0 and len(json_errors) == 0:
        readiness_verdict = "A. READY FOR TRAINING"
    else:
        readiness_verdict = "B. NEED MORE DATA"

    print("\n" + "=" * 80)
    print(f"AQPG PHASE 14 QUALITY GATES VERDICT: {overall_status}")
    ...
    print(f"\n[SUCCESS] Saved Phase 14 Audit JSON to: {AUDIT_JSON_PATH}")

    # Build Text Audit Report
    report_lines = [
        "================================================================================",
        "AQPG PHASE 14 DATASET FORENSIC AUDIT REPORT",
        "================================================================================",
        ...
        f"  - Total V14 Records:    {total_records}",
    ...
    with open(AUDIT_REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f"[SUCCESS] Saved Phase 14 Audit Report to: {AUDIT_REPORT_PATH}")
