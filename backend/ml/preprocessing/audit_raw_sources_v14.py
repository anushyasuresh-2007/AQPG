"""
audit_raw_sources_v14.py
AQPG Phase 14 Raw Dataset Forensic Inventory Engine.

Scans all raw files in `datasets/raw/v14/` and extracted archives.
Generates:
  - datasets/phase14_source_inventory.json
  - datasets/phase14_source_inventory_report.txt
"""
...
RAW_V14_DIR = os.path.join(RAW_DIR, "v14")
EXTRACTED_DIR = os.path.join(DATASETS_DIR, "extracted")

JSON_OUT = os.path.join(DATASETS_DIR, "phase14_source_inventory.json")
REPORT_OUT = os.path.join(DATASETS_DIR, "phase14_source_inventory_report.txt")
...
def run_v14_raw_inventory():
    print("=" * 80)
    print("AQPG PHASE 14 RAW DATASET FORENSIC INVENTORY")
    print("=" * 80)
    
    inventory = []
    
    # 1. Scan Phase 14 JSON files in datasets/raw/v14
    v14_sources = [
    ...
    for fn, name, url in v14_sources:
        p = os.path.join(RAW_V14_DIR, fn)
        if os.path.exists(p):
    ...
    print(f"\n[SUCCESS] Saved Phase 14 JSON inventory to: {JSON_OUT}")

    # Save Text Inventory Report
    report_lines = [
        "================================================================================",
        "AQPG PHASE 14 RAW SOURCE INVENTORY REPORT",
        "================================================================================",
    ...
    with open(REPORT_OUT, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f"[SUCCESS] Saved Phase 14 Text inventory report to: {REPORT_OUT}")

if __name__ == "__main__":
    run_v14_raw_inventory()
