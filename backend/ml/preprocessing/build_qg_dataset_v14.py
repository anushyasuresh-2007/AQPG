"""
build_qg_dataset_v14.py
AQPG Phase 14 Master Dataset Ingestion, Normalization, Verification & Rebuild Engine.

Executes 20 Sequential Pipeline Stages:
  1. Source Discovery across raw archives and datasets/raw/v14/
  2. Source Ingestion
  ...
  19. Master V14 Dataset Assembly (datasets/v14/qg_dataset_v14.jsonl)
  20. Deterministic Leakage-Safe 80/20 Train/Val Split (seed 42)

Outputs:
  - datasets/v14/qg_dataset_v14.jsonl
  - datasets/v14/qg_train_dataset_v14.jsonl
  - datasets/v14/qg_validation_dataset_v14.jsonl
  - datasets/v14/qg_dataset_v14_report.txt
"""
...
RAW_V14_DIR = os.path.join(DATASETS_DIR, "raw", "v14")
V14_DIR = os.path.join(DATASETS_DIR, "v14")

os.makedirs(V14_DIR, exist_ok=True)

MASTER_V14_PATH = os.path.join(V14_DIR, "qg_dataset_v14.jsonl")
TRAIN_V14_PATH = os.path.join(V14_DIR, "qg_train_dataset_v14.jsonl")
VAL_V14_PATH = os.path.join(V14_DIR, "qg_validation_dataset_v14.jsonl")
REPORT_V14_PATH = os.path.join(V14_DIR, "qg_dataset_v14_report.txt")
...
def construct_v14_prompt(rec: dict) -> str:
    """Constructs structured encoder prompt for V14 Master Schema."""
...
def run_v14_pipeline():
    print("=" * 80)
    print("AQPG PHASE 14 MASTER DATASET PIPELINE (20 STAGES)")
    print("=" * 80)

    all_raw_records = []

    # 1. Ingest Phase 14 raw JSON files from datasets/raw/v14
    v14_files = [
            ...
            with open(p, "r", encoding="utf-8") as f:
                ...
                for item in items:
                    all_raw_records.append({
                        ...
                        "board": item.get("board", "Public Benchmark" if "ncert" not in vf else "CBSE"),
                        ...
                        "provenance": item.get("provenance", "Phase 14 Ingested Dataset"),
    ...
    for idx, r in enumerate(all_raw_records, 1):
        ...
        master_rec["input_text"] = construct_v14_prompt(master_rec)
        v14_records.append(master_rec)

    total_v14 = len(v14_records)
    print(f"\nSuccessfully Constructed {total_v14} Clean V14 Records.")
    print("Rejection Breakdown:")
    for reason, count in rejection_stats.items():
        print(f"  - {reason}: {count}")

    # Save Master V14 Dataset
    with open(MASTER_V14_PATH, "w", encoding="utf-8") as f:
        for r in v14_records:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")
    print(f"\n[SUCCESS] Saved Master V14 to: {MASTER_V14_PATH} ({total_v14} records)")

    # Deterministic Leakage-Safe 80/20 Train / Val Split (seed 42)
    random.seed(42)
    shuffled = v14_records.copy()
    random.shuffle(shuffled)
    
    split_idx = int(0.80 * total_v14)
    train_v14 = shuffled[:split_idx]
    val_v14 = shuffled[split_idx:]

    with open(TRAIN_V14_PATH, "w", encoding="utf-8") as f:
        for r in train_v14:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")

    with open(VAL_V14_PATH, "w", encoding="utf-8") as f:
        for r in val_v14:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")

    print(f"[SUCCESS] Saved Train V14 to:  {TRAIN_V14_PATH} ({len(train_v14)} records)")
    print(f"[SUCCESS] Saved Val V14 to:    {VAL_V14_PATH} ({len(val_v14)} records)")

    # Build Report
    subject_counts = Counter(r["subject"] for r in v14_records)
    class_counts = Counter(r["class"] for r in v14_records)
    board_counts = Counter(r["board"] for r in v14_records)
    status_counts = Counter(r["verification_status"] for r in v14_records)

    report_lines = [
        "================================================================================",
        "AQPG PHASE 14 MASTER DATASET PIPELINE BUILD REPORT",
        "================================================================================",
        f"Total Ingested Raw Candidate Records: {len(all_raw_records)}",
        f"Total Clean V14 Records Constructed:  {total_v14}",
        f"Total Rejected Records:               {len(all_raw_records) - total_v14}\n",
        "TRAIN / VAL LEAKAGE-SAFE SPLIT (Seed 42):",
        f"  - Training Set V14 (80%):    {len(train_v14)} records",
        f"  - Validation Set V14 (20%):  {len(val_v14)} records\n",
        "SUBJECT DISTRIBUTION IN V14:"
    ]
    for sb, count in subject_counts.items():
        report_lines.append(f"  - {sb}: {count} ({(count/total_v14*100):.2f}%)")

    report_lines.extend(["\nCLASS DISTRIBUTION IN V14:"])
    for cl, count in class_counts.items():
        report_lines.append(f"  - {cl}: {count} ({(count/total_v14*100):.2f}%)")

    report_lines.extend(["\nBOARD DISTRIBUTION IN V14:"])
    for bd, count in board_counts.items():
        report_lines.append(f"  - {bd}: {count} ({(count/total_v14*100):.2f}%)")

    report_lines.extend(["\nVERIFICATION STATUS BREAKDOWN IN V14:"])
    for st, count in status_counts.items():
        report_lines.append(f"  - {st}: {count} ({(count/total_v14*100):.2f}%)")

    with open(REPORT_V14_PATH, "w", encoding="utf-8") as f:
        f.write("\n".join(report_lines))
    print(f"[SUCCESS] Saved Pipeline Build Report to: {REPORT_V14_PATH}")
